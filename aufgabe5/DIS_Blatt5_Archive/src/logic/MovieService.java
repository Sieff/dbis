...
public class MovieService extends MovieServiceBase {
	...
	public MovieService() {
		...
		// Create a GriFS FileSystem Object using the
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		//TODO : implement
		FindIterable<Document>  result = movies
				.find(and(gt("votes", minVotes), gt("rating", minRating)))
				.sort(Sorts.descending("rating", "votes"))
				.limit(limit);
		return result;
	}
	...
	public void saveMovieComment(String id, String comment) {
		...
		movies.updateOne(query, updates);
	}
	...
	public FindIterable<Document> searchTweets(String query) {
		...
		FindIterable<Document> result = tweets.find(filter);
	...
	public FindIterable<Document>  getGeotaggedTweets(int limit) {
		//TODO : implement
		FindIterable<Document> result = tweets.find(and(exists("coordinates"), not(eq("coordinates", null))))
				.limit(limit);
		return result;
	}
